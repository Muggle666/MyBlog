...
Cglib包弥补了JDK动态代理必须依赖接口实现的不足，但并不是所有的目标对象都会继承父类或者实现接口。CGLib 采用了非常底层的字节码技术，其原理是通过字节码技术为一个类创建子类，并在子类中采用方法拦截的技术拦截所有父类方法的调用，顺势织入横切逻辑，来完成动态代理的实现。但由于CGLib采用动态创建子类的方法，对于final方法，无法进行代理。
