...
静态代理作为原始的代理模式设计，理解到设计的思想之后会发现代理模式的好处；而动态代理则弥补了静态代理需要子类继承或者实现父类的缺点，利用反射机制创建对象，进一步降低了耦合度，但并不是所有的目标对象都会继承父类或者实现接口，这时还好Cglib包弥补了不足。我们可以根据实际的开发场景选择动态代理或者使用Cglib代理。
