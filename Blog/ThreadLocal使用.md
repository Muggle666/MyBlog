...
也就是说，每个线程内部都会有ThreadLocalMap类型的局部变量，用于记录ThreadLocal声明的变量，而且栈帧的局部变量都是ThreadLocal声明的变量的副本，所以线程内部改变ThreadLocal的变量也不会影响其他线程，也不存在并发的。如下图所示：
