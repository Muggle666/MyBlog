...
单例模式的实现：
- <a href="#lazyAndHungry">懒汉模式和饿汉模式</a>
<a href="#debugOfMultithreading">多线程debug</a>
...
#### <a name="distinction">懒汉模式 vs 饿汉模式</a>
...
<a name="lazyAndHungry">懒汉模式和饿汉模式示例：</a>
```java
/**
 * 懒汉模式（延迟加载，非线程安全）
 */
...
##### <a name="debugOfMultithreading">*在重现问题之前，首先要学会多线程debug。可以参考[ idea 多线程debug](https://blog.csdn.net/kevindai007/article/details/71412324)</a>
...
<a name="doubleLock">修改后的单例模式是否还能优化呢？接下来介绍双重校验加锁机制。废话不多说，上代码！</a>
```java
/**
  * 双重校验加锁（延迟加载，线程安全）
  */
...
<a name="innerClass">那有没有一种方法，既可以保证线程安全，又能延迟加载呢？</a>
使用内部类实现单例模式(**推荐使用**)：
```java
...
### <a name="serializationAttack">序列化对单例模式的破坏</a>
...
### <a name="reflectAttack">反射对单例类的攻击</a>
