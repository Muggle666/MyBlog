...
接下来看下ThreadLocal类的set()相关源码，值得注意的是，由于Entry对象是继承WeakReference，所以Entry对象是弱引用的，简单来说就是很容易被GC回收，所以在ThreadLocal类中，大部分方法都涉及判断对象是否为null，如果为null就要从数组中移除，避免内存溢出。因此ThreadLocal类源码比较长，对于移除Entry对象的方法简单理解就可以，
```java
